    ...
    public class UpdateBrandCommandHandler : IRequestHandler<UpdateBrandCommand, UpdatedBrandResponse>
    {
        ...
        public async Task<UpdatedBrandResponse> Handle(UpdateBrandCommand request, CancellationToken cancellationToken)
        {
            Brand? brand = await _brandRepository.GetAsync(
                ...
                cancellationToken: cancellationToken); // Sistemimiz de böyle bir data var mı ?

            brand = _mapper.Map(request, brand); // request ile yolladığımız değerleri brand'e taşıyacaktır bu nokta da
